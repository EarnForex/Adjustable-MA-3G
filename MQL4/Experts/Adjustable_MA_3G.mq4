...
void UpdatePanel()
{
    ...
    double fma = iCustom(NULL, 0, "3rdGenMA", FastMA, Period_Sampling_Fast, MA_Method_Fast, MA_Applied_Price_Fast, 0, 1);
    double sma = iCustom(NULL, 0, "3rdGenMA", SlowMA, Period_Sampling_Slow, MA_Method_Slow, MA_Applied_Price_Slow, 0, 1);
        ...
        if ((OrderMagicNumber() == Magic) && (OrderSymbol() == Symbol()))
        {
            if (OrderType() == OP_BUY)
            {
                ...
                if (Bid - OrderOpenPrice() >= TriggerDistance * _Point)
                {
                    ...
                    if ((Bid - TrailingStop * _Point) - OrderStopLoss() > _Point / 2) // Double-safe comparison.
            ...
            else if (OrderType() == OP_SELL)
            {
                ...
                {
                    // If the current stop-loss is below the desired trailing stop level.
                    if ((OrderStopLoss() - (Ask + TrailingStop * _Point) > _Point / 2) || (OrderStopLoss() == 0)) // Double-safe comparison.

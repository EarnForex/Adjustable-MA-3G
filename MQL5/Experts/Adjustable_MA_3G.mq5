...
//|                             Copyright © 2011-2026, EarnForex.com |
//|                                       https://www.earnforex.com/ |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2011-2026, EarnForex"
...
#property tester_indicator "3rdGenMA"
...
input int MinDiff = 10; // Minimum difference between MAs in points
input int StopLoss = 1700; // Stop-loss in points
...
int OnInit()
{
    ...
    {
        Print("Period_Sampling_Slow should be >= Period_Slow * 4. Period_Sampling_Slow = ", Period_Sampling_Slow, ", SlowMA = ", SlowMA, ", minimum = ", SlowMA * 4, ".");
        return INIT_FAILED;
    }
    if (Period_Sampling_Fast < FastMA * 4)
    {
        Print("Period_Sampling_Fast should be >= Period_Fast * 4. Period_Sampling_Fast = ", Period_Sampling_Fast, ", FastMA = ", FastMA, ", minimum = ", FastMA * 4, ".");
        return INIT_FAILED;
    }
    ...
    {
        Print("Failed to load custom indicator 3rdGenMA (fast). Period = ", FastMA, ", Sampling = ", Period_Sampling_Fast, ", error: ", GetLastError());
        return INIT_FAILED;
    }
    ...
    {
        Print("Failed to load custom indicator 3rdGenMA (slow). Period = ", SlowMA, ", Sampling = ", Period_Sampling_Slow, ", error: ", GetLastError());
        return INIT_FAILED;
    }
...
void UpdatePanel()
{
    ...
    if (CopyBuffer(myFastMA, 0, 1, 1, FMABuffer) != 1) return;
    if (CopyBuffer(mySlowMA, 0, 1, 1, SMABuffer) != 1) return;
...
void InitPrevCross()
{
    ...
    if (CopyBuffer(myFastMA, 0, 2, count, FMABuffer) <= 0) return;
    if (CopyBuffer(mySlowMA, 0, 2, count, SMABuffer) <= 0) return;
